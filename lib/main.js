/**
 * Primary ad tag (i.e. CLoader) code.
 */
...
module.exports = function(){
    ...
    var _getClickTrackingUrl = function(context){
        ...
        // Search for macros (`${MACRONAME}`) in base URL and replace with macro values
        // from adserver & adexchange data
        // Keys are assumed to be simply "MACRONAME".toLowerCase(), so in this case 'macroname'.
        var macroPattern = /\${([A-Z-_]*)}/g;
        var match;
        while ((match = macroPattern.exec(url)) != null) {
            ...
            if (context.exchangeAuctionData.hasOwnProperty(field)){
                url = url.replace(match[0], context.exchangeAuctionData[field])
            } else if (context.adserverAuctionData.hasOwnProperty(field)){
                url = url.replace(match[0], context.adserverAuctionData[field])
            } else if (field === 'crextid'){
                url = url.replace(match[0], context.creativeExternalId);
            } else if (field === 'campextid'){
                url = url.replace(match[0], context.campaignExternalId);
            }
        }
        ...
        if (context.external){
            ...
            // with '&'
            url += (url.search('\\?') > -1 ? '&' : '?') + serializeObject(context.external);
        }
    ...
    var _templatePostRender = function(targetElement, context, dims){
        ...
        images.forEach(function(image){
            ...
            window.setTimeout(function(){
                ...
                var minHeight = context.minImageHeight || 100;
                var minWidth = context.minImageWidth || 100;
